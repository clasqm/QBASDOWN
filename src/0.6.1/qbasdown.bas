...
REM this apparently even works with the REM in place!
REM $lang: "qb"
REM
...
SUB ChkFCB (Fence$)
    REM opening line
    IF CodeBlock% = 0 THEN
        IF LEFT$(CurrentLine$, 3) = Fence$ THEN
    ...
    ELSEIF CodeBlock% = 1 THEN
        REM closing line
        REM for some reason the line below simply will not work with
        REM LEFT$(CurrentLine$, 3) = Fence$
        REM it is just plain weird
...
SUB GetFileSize
    ...
    WEND
    IF Verbose% = 1 THEN PRINT "Total of " + STR$(LineCounter%) + " lines."
    CLOSE #1
END SUB
