...
COMMON SHARED printed%, skip%, LineCounter%, Verbose%, CodeBlock%, ListOn%, OListOn%, RefsNum%, EndNotes%
...
SUB ChkPandocHeaders
	OPEN InFile$ FOR INPUT AS #1
		...
		LINE INPUT #1, a$
		IF LEFT$(a$, 2) = "% " THEN
			 PRINT #2, "<meta=" + CHR$(34) + "author" + CHR$(34) + " content=" + CHR$(34) + MID$(a$, 3) + CHR$(34) + ">"
			 skip% = skip% + 1
		END IF
	CLOSE #1
END SUB
...
SUB MakeOutFile
	...
	PRINT #2, "</style>"
	ChkPandocHeaders
	PRINT #2, "<meta=" + CHR$(34) + "GENERATOR" + CHR$(34) + " content=" + CHR$(34) + "QBASDOWN" + CHR$(34) + " > "
	PRINT #2, "</head>"
	PRINT #2, "<body>"
END SUB
