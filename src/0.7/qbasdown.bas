...
COMMON SHARED PastLine$, CurrentLine$, NextLine$, Infile$, OutFile$, AllReferences$, AllEndNotes$
OPTION BASE 1
REM
...
FOR f = 1 TO LineCounter% + 1
    ...
    IF UCASE$(RTRIM$(LTRIM$(CurrentLine$))) = "\BIB" THEN PrintBib
    REM
...
SUB GetFileSize
    ...
    WHILE NOT (EOF(1))
        LINE INPUT #1, a$
        IF UCASE$(LEFT$(a$, 3)) <> "\EN" THEN LineCounter% = LineCounter% + 1
    WEND
